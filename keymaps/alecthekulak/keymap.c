...
void copy_search(void) {  // https://github.com/tmk/tmk_keyboard/issues/332#:~:text=No%20action-,WSCH,-Focus%20address%20bar
  ...
  // if (conf.key_pressed.so == false) { tap_code16(LCTL(KC_C)); } 
  // tap_code_delay(KC_LEFT_GUI, 1500);
  clear_mods();
  uprintf("%u Clicking windows button...\n", timer_elapsed(timer)); tap_code_delay(KC_WWW_SEARCH, 400); // KC_LEFT_GUI  
  // // KC_APPLICATION -> "Windows Context Menu" -> Right clicking on a webpage, etc. 
  // SEND_STRING(SS_TAP(X_WWW_SEARCH) SS_DELAY(400) "Chrome" SS_DELAY(225) SS_TAP(X_ENTER) SS_DELAY(500)); // tap_code_delay(KC_RIGHT_GUI, 400);
  uprintf("%u Typing 'Google Chrome'...\n", timer_elapsed(timer));
  SEND_STRING("Chrome" SS_DELAY(225) SS_TAP(X_ENTER) "" SS_DELAY(500));
  // uprintf("%u Hitting 'ENTER'...\n", timer_elapsed(timer));  // tap_code_delay(KC_ENTER, 500);
  uprintf("%u Hitting 'CTRL+V'...\n", timer_elapsed(timer)); tap_code16(LCTL(KC_V)); 
  uprintf("%u Hitting 'ENTER'...\n", timer_elapsed(timer)); tap_code(KC_ENTER);
  uprintf("%u Finishing up...\n", timer_elapsed(timer)); // return true;
}
uint32_t _delay_copy_search(uint32_t trigger_time, void *cb_arg) {  //void *cb_arg
    // uprintln("\n --------- THIS IS THE DEFFERED ACTION ---------");
    copy_search(); // uprintln(" --------- FINISHED:   DEFFERED ACTION ---------\n");
    user_config.pending_action = INVALID_DEFERRED_TOKEN; log_key('|');  //user_config.action_taken = true; // print_uconfig();
...
uint32_t _delay_c(uint32_t trigger_time, void *cb_arg) {  //void *cb_arg
    ...
    user_config.pending_action = INVALID_DEFERRED_TOKEN; log_key('|');  //user_config.action_taken = true; 
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  // #ifdef CONSOLE_ENABLE  // If console is enabled, it will print the matrix position and status of each key pressed
